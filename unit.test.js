...
// ЗМІНА: Імпортуємо app, server та client (mqtt), щоб коректно їх закрити
// Якщо виникає помилка "app is not a function", перевір, чи є в server.js рядок: 
...
describe('Unit & Integration Tests (Server Module)', () => {
    ...
    // --- ТВОЇ ОРИГІНАЛЬНІ ТЕСТИ (БЕЗ ЗМІН) ---
    ...
    // --- НОВИЙ БЛОК ДЛЯ ВИПРАВЛЕННЯ ПОМИЛКИ ---

    // Це вирішує проблему: "ReferenceError: You are trying to import a file after the Jest environment has been torn down"
    afterAll((done) => {
        const closeMqtt = () => {
